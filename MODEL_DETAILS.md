# Детали реализации моделей

## Содержание
1. [Прогнозирование уровня стресса](#1-прогнозирование-уровня-стресса)
2. [Генерация рекомендаций](#2-генерация-рекомендаций)
3. [Технические решения](#3-технические-решения)

[📚 Описание проекта](README.md)

---

**Почему Tensorflow?**
>TensorFlow предоставляет готовые инструменты для сквозного ML-пайплайна — от экспериментов с архитектурой до продакшн-деплоя, что критично для проектов с ограниченными данными.
> Оптимизированная и быстрая работа на CPU. 

## 1. Прогнозирование уровня стресса

### Архитектура модели
```python
model = tf.keras.Sequential([
    tf.keras.layers.Dense(64, activation='relu', input_shape=(13,)),
    tf.keras.layers.BatchNormalization(),
    tf.keras.layers.Dropout(0.3),
    tf.keras.layers.Dense(32, activation='relu', kernel_regularizer=tf.keras.regularizers.l1_l2(0.01)),
    tf.keras.layers.Dense(1)
])
```
### Слой за слоем:
**Входной слой:** 64 нейрона с ReLU + L2-регуляризация (λ=0.01)

**Batch Normalization:** Стандартизация активаций предыдущего слоя

**Dropout (30%):** Исключение случайных нейронов для борьбы с переобучением

**Скрытые слои:** 32 нейрона с ReLU + комбинированная L1/L2-регуляризация (λ=0.01)

**Выход:** 1 нейрон с линейной активацией для регрессии

### Параметры обучения
```python
checkpoint = tf.keras.callbacks.ModelCheckpoint(
    'models/stress_model.keras',
    save_best_only=True,
    monitor='val_loss',
    mode='min'
)

lr_scheduler = tf.keras.callbacks.ReduceLROnPlateau(
    monitor='val_loss',
    factor=0.5,
    patience=10
)

history = model.fit(
    X_train, y_train,
    epochs=200,
    batch_size=64,
    validation_split=0.2,
    callbacks=[checkpoint, lr_scheduler],
    verbose=1
)
```

### Особенности
**Тип задачи:** Регрессия

**Регуляризация:**

- L2-регуляризация в первом слое

- Смешанная L1/L2-регуляризация во втором скрытом слое

**Оптимизация обучения:**

- Автоматическое уменьшение learning rate при плато (пациент=10 эпох)

- Пакетная нормализация между слоями

- Сохранение лучшей модели по val_loss

**Метрики качества:**

- MAE (Mean Absolute Error)
- MSE (Mean Squared Error)

**Стратегия валидации:**

- 20% данных выделено для валидации

- Тестовый набор: 20% от исходных данных

---

## 2. Генерация рекомендаций
### Архитектура модели
```python
model = tf.keras.Sequential([
    tf.keras.layers.Dense(32, activation='relu', input_shape=(1,)),
    tf.keras.layers.Dropout(0.3),
    tf.keras.layers.Dense(16, activation='relu'),
    tf.keras.layers.Dense(y.shape[1], activation='sigmoid')
])
```

### Слой за слоем:
**Вход:** Нормализованный уровень стресса (0-1)

**Скрытые слои:** 32 нейрона с ReLU

**Dropout (30%):** Случайное отключение нейронов для борьбы с переобучением

**Второй скрытый слой:** 16 нейронов с ReLU

**Выход:** y.shape[1] нейронов (сигмоида для независимой бинарной классификации)

### Параметры обучения

```python
model.compile(
    optimizer=tf.keras.optimizers.Adam(learning_rate=0.005),
    loss='binary_crossentropy',
    metrics=[
        tf.keras.metrics.Precision(name='precision'),
        tf.keras.metrics.Recall(name='recall'),
        tf.keras.metrics.AUC(name='auc')
    ]
)

early_stopping = tf.keras.callbacks.EarlyStopping(
    monitor='val_loss',
    patience=5,
    restore_best_weights=True
)
```

### Особенности
**Тип задачи:** ММульти-лейбл классификация механизмов борьбы со стрессом

**Ключевые моменты:**

- Добавление Dropout (30%) для регуляризации
- Динамический выходной слой под размер целевых данных (y.shape[1])

**Метрики оценки:**

- Precision: Доля релевантных рекомендаций
- Recall: Покрытие всех необходимых рекомендаций
- AUC: Качество разделения классов

**Оптимизация обучения:**

- Ранняя остановка при отсутствии улучшений 5 эпох
- Адаптивный learning rate (стартовое значение 0.005)

**Стратегия валидации:**

- Тестовый набор: 20% исходных данных
- Валидация на обучении: 20% от тренировочного набора

**Особенности данных:**

- Целевые переменные: бинарные векторы рекомендаций
- Нормализация входного признака (Mental Stress Level) в диапазон [0, 1]

**Классы рекомендаций (пример):**

- Медитация
- Физические упражнения
- Консультация специалиста
- Коррекция сна
- Социальная активность
- Тайм-менеджмент
- Диетические рекомендации
- Финансовый план

---


## 3. Технические решения

### Работа с малым объемом данных (20 000 samples)
**Стратегии предотвращения переобучения:**
-  **Аугментация данных**: 
  - Регуляризация L1/L2 для контроля весов
  - Dropout слои (30%) для уменьшения коадаптации нейронов
  - Batch Normalization для стабилизации обучения
-  **Оптимизация обучения:**: 
  - Adaptive Learning Rate (ReduceLROnPlateau)
  - Early Stopping с восстановлением лучших весов
-  **Балансировка данных:**:
  - Стратифицированное разделение выборки
  - Автоматическое взвешивание классов

### Этапы препроцессинга:
1. **Трансформация признаков**

- Бинаризация категориальных переменных:
  - Gender: Male → 1, Female → 0
  - Counseling Attendance: Yes → 1, No → 0
- Мульти-хот кодирование для механизмов преодоления стресса
- Создание синтетических признаков:
  - total_weekly_hours: сумма учебных и рабочих часов
  - sleep_efficiency: отношение сна к использованию соцсетей

2. **Обработка данных**

- Удаление нефункциональных идентификаторов (User ID)
- Стандартизация числовых признаков (Z-score нормализация)
- Автоматическое удаление дубликатов (5% от датасета)

3. **Сохранение данных:**

- Разделение на 2 потока:
  - stress_data.csv: 13 признаков + целевая переменная (регрессия)
  - mechanism_data.csv: 1 признак + 10 бинарных меток (многозадачная классификация)

### Сравнение архитектур моделей

| Параметр      | Модель стресса                           | Модель рекомендаций                       |
|---------------|------------------------------------------|-------------------------------------------|
| Архитектура   | 64-32-1 Dense                            | 32-16-10 Dense                            |
| Активации     | ReLU (скрытые слои),<br/> Linear (выход) | ReLU (скрытые слои),<br/> Sigmoid (выход) |
| Оптимизатор   | Adam (lr=0.001)                          | Adam (lr=0.005)                           |
| Регуляризация | L2 (0.01) + Dropout                      | Dropout (30%)                             |
| Метрики       | MSE, MAE                                 | Precision, Recall, AUC                    |
| Обучение      | 200 эпох (пакет 64)                      | L50 эпох (пакет 32)                       |
| Колбеки       | ModelCheckpoint + ReduceLROnPlateau      | EarlyStopping                             |


**Особенности обработки:**

- Для регрессии:
  - Нормализация StandardScaler
  - Динамическое изменение learning rate
  - Валидация на 20% тренировочных данных
  
- Для классификации:
  - Нормализация целевой переменной (0-10 → 0-1)
  - Многозадачная бинарная классификация
  - Пороговая обработка выходных значений (sigmoid > 0.5)

**Визуализация результатов:**

- Регрессия:
    - График сравнения предсказаний с реальными значениями
    - Анализ кривых обучения (train/val loss)

- Классификация:
  - Мониторинг метрик precision-recall
  - Визуализация ROC-кривых для каждого механизма

**Оптимизация производительности:**

- Кэширование предобработанных данных
- Пайплайн обработки в памяти
- Пакетная загрузка данных с префетчингом
- Автоматическое распределение ресурсов TensorFlow
